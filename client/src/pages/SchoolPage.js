...
import { Typography, Button, CardMedia, CssBaseline, Drawer, Grid, Box, Paper, FormControl, TextField } from "@material-ui/core";
...
const SchoolPage = (props) => {
  ...
  const updateSchool = e => {
    ...
    fetch('/school/', {
      ...
      body: JSON.stringify({ schoolName, about, location, admission, image_url })
    })
      .then(res => {
        status = res.status;
          ...
          <Box className={classes.updateBox} border={1}>
            <Grid container>
              <form className={classes.form}>
                <Grid container justify='space-between' direction='row'>
                  ...
                  <div className={classes.container}>
                    <FormControl className={classes.formC}>
                      {card}
                      <img className={classes.schoolImage} src={String(schoolData['image_url']).slice(1,-1)} />
                    </FormControl>
                  </div>
                </Grid>
                {err.length > 0 && (
                ...
                <Typography>
                  <h4>Admission</h4>
                  <p>{schoolData.admission}</p>
                </Typography>
                <Grid container justify='center'>
                  <Button href={`/update-school/${window.location.href.substring(window.location.href.lastIndexOf('/') + 1)}`} className={classes.submit}>
                    Update
                  </Button>
                </Grid>
              </form>
